...
class AmerisPipeline:

    # Database setup
    conn = sqlite3.connect('ameris.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ameris`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ameris WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ameris (date, title, link, content)"
